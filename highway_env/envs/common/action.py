...
class DiscreteAction(ContinuousAction):
    def __init__(self,
        ...
        axes = np.linspace(cont_space.low, cont_space.high, self.actions_per_axis, axis=1)
        self.all_actions = list(product(*axes))

    def print_action_info(self):
        print(f'[INFO] all actions: {self.all_actions}')

    def space(self) -> spaces.Discrete:
        return spaces.Discrete(self.actions_per_axis**self.size)

    def act(self, action: int) -> None:
        super().act(self.all_actions[action])
